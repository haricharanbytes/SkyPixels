...
ipvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate ipvi

ipvi = np.where((nir+red)==0., 0,  (nir)/(nir + red))  

ipviImage = rasterio.open('ipvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
ipviImage.write(ipvi,1)
ipviImage.close()

# Open the TIFF file
with rasterio.open('ipvi.tif') as src:
    # Read the file into a NumPy array
    ipvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
ipvi_min = np.min(ipvi_data)
ipvi_max = np.max(ipvi_data)

ipvi = rasterio.open('ipvi.tif')
plot.show(ipvi, title='ipvi')
