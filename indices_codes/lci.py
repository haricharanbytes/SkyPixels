...
lci = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate lci

lci = np.where((nir+rededge)==0., 0,  (nir - rededge)/(nir + rededge))  

lciImage = rasterio.open('lci.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
lciImage.write(lci,1)
lciImage.close()

# Open the TIFF file
with rasterio.open('lci.tif') as src:
    # Read the file into a NumPy array
    lci_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
lci_min = np.min(lci_data)
lci_max = np.max(lci_data)

lci = rasterio.open('lci.tif')
plot.show(lci, title='lci')
