...
rendvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate rendvi

rendvi = np.where((nir+rededge)==0., 0,  (nir - rededge)/(nir + rededge))  

rendviImage = rasterio.open('rendvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
rendviImage.write(rendvi,1)
rendviImage.close()

# Open the TIFF file
with rasterio.open('rendvi.tif') as src:
    # Read the file into a NumPy array
    rendvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
rendvi_min = np.min(rendvi_data)
rendvi_max = np.max(rendvi_data)

rendvi = rasterio.open('rendvi.tif')
plot.show(rendvi, title='rendvi')
