...
chI = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate chI

chI = np.where((nir / green)==0., 0,  (nir / green) - 1)  

chIImage = rasterio.open('chI.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
chIImage.write(chI,1)
chIImage.close()

# Open the TIFF file
with rasterio.open('chI.tif') as src:
    # Read the file into a NumPy array
    chI_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
chI_min = np.min(chI_data)
chI_max = np.max(chI_data)

chI = rasterio.open('chI.tif')
plot.show(chI, title='chI')
