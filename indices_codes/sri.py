...
sri = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate sri

sri = np.where((red)==0., 0,  (nir)/(red))  

sriImage = rasterio.open('sri.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
sriImage.write(sri,1)
sriImage.close()

# Open the TIFF file
with rasterio.open('sri.tif') as src:
    # Read the file into a NumPy array
    sri_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
sri_min = np.min(sri_data)
sri_max = np.max(sri_data)

sri = rasterio.open('sri.tif')
plot.show(sri, title='sri')
