...
wdrvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate wdrvi

wdrvi = np.where((0.2*nir+red)==0., 0,  (0.2 * nir - red)/(0.2 * nir + red))  

wdrviImage = rasterio.open('wdrvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
wdrviImage.write(wdrvi,1)
wdrviImage.close()

# Open the TIFF file
with rasterio.open('wdrvi.tif') as src:
    # Read the file into a NumPy array
    wdrvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
wdrvi_min = np.min(wdrvi_data)
wdrvi_max = np.max(wdrvi_data)

wdrvi = rasterio.open('wdrvi.tif')
plot.show(wdrvi, title='wdrvi')
