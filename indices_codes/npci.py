...
npci = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate npci

npci = np.where((red + blue)==0., 0,  (red - blue)/(red + blue))  

npciImage = rasterio.open('npci.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
npciImage.write(npci,1)
npciImage.close()

# Open the TIFF file
with rasterio.open('npci.tif') as src:
    # Read the file into a NumPy array
    npci_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
npci_min = np.min(npci_data)
npci_max = np.max(npci_data)

npci = rasterio.open('npci.tif')
plot.show(npci, title='npci')
