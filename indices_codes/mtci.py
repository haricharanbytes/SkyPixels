...
rededgeband.height
rededgeband.width
rededgeband.dtypes[0]
rededgeband.crs
rededgeband.transform
rededgeband.read(1)
...
mtci = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate mtci

mtci = np.where((rededge - red)==0., 0,  (nir - rededge)/(rededge - red))  
mtci = np.clip(mtci, -1, 1)

mtciImage = rasterio.open('mtci.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
mtciImage.write(mtci,1)
mtciImage.close()

# Open the TIFF file
with rasterio.open('mtci.tif') as src:
    # Read the file into a NumPy array
    mtci_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
mtci_min = np.min(mtci_data)
mtci_max = np.max(mtci_data)

mtci = rasterio.open('mtci.tif')
plot.show(mtci, title='mtci')
