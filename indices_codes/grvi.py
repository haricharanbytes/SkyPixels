...
# Create empty matrix
grvi = np.empty(nir.shape, dtype=rasterio.float32)

# Calculate grvi
grvi = np.where((green + red) == 0., 0, (green - red) / (green + red))

# Save grvi to a new file
grviImage = rasterio.open(
    'grvi.tif', 'w',
    driver='GTiff',
...
# Open the TIFF file
with rasterio.open('grvi.tif') as src:
    # Read the file into a NumPy array
    grvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
grvi_min = np.min(grvi_data)
grvi_max = np.max(grvi_data)

grvi = rasterio.open('grvi.tif')
plot.show(grvi, title='grvi')
