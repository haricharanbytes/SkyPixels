...
exgvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate exgvi

exgvi = np.where((2*green - red - blue)==0., 0,  (2*green - red - blue))

exgviImage = rasterio.open('exgvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
exgviImage.write(exgvi,1)
exgviImage.close()

# Open the TIFF file
with rasterio.open('exgvi.tif') as src:
    # Read the file into a NumPy array
    exgvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
exgvi_min = np.min(exgvi_data)
exgvi_max = np.max(exgvi_data)

exgvi = rasterio.open('exgvi.tif')
plot.show(exgvi, title='exgvi')
