...
arvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate arvi

arvi = np.where((nir + 2*(red) - blue)==0., 0,  (nir - (2 * red) + blue)/(nir + (2 * red) - blue))  

arvi = np.clip(arvi, -1, 1)
arviImage = rasterio.open('arvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float32')
arviImage.write(arvi,1)
arviImage.close()

# Open the TIFF file
with rasterio.open('arvi.tif') as src:
    # Read the file into a NumPy array
    arvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
arvi_min = np.min(arvi_data)
arvi_max = np.max(arvi_data)

arvi = rasterio.open('arvi.tif')
plot.show(arvi, title='arvi')
