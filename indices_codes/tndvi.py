...
tndvi = np.empty(red.shape, dtype=rasterio.float32)  # Create empty matrix


# calculate tndvi

tndvi = np.where((nir+red)==0., 0,  np.sqrt(((nir-red)/(nir+red))+0.5)  

tndviImage = rasterio.open('tndvi.tif','w',driver = 'GTiff',width=redband.width, height=redband.height, count=1 ,crs=redband.crs ,transform=redband.transform ,dtype='float64')
tndviImage.write(tndvi,1)
tndviImage.close()

# Open the TIFF file
with rasterio.open('tndvi.tif') as src:
    # Read the file into a NumPy array
    tndvi_data = src.read(1)  # assuming the data is in the first band

# Calculate the minimum and maximum values
tndvi_min = np.min(tndvi_data)
tndvi_max = np.max(tndvi_data)

tndvi = rasterio.open('tndvi.tif')
plot.show(tndvi, title='tndvi')
